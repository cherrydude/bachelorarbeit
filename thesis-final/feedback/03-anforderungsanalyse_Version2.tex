...
Die Anwendung unterstützt neben der direkten Bearbeitung auch die persistente Speicherung sowie den JSON-basierten Import und Export von Szenarien. Ein Szenario kann zusammen mit den zugehörigen Simulationsläufen exportiert werden. Beim Import werden sowohl die Task-Daten und Verfahrensparameter als auch die gespeicherten Runs wiederhergestellt.
...
\begin{itemize}
    ...
    \item \textbf{FR-12:} Ein Szenario muss zusammen mit den zugehörigen Simulationsläufen in einem JSON-basierten Format exportiert werden können.
    \item \textbf{FR-13:} Exportierte Szenarien einschließlich ihrer Simulationsläufe müssen wieder importiert werden können.
\end{itemize}
...
Szenarien und die dazugehörigen Simulationsläufe müssen als JSON-Datei exportiert und zu einem späteren Zeitpunkt wieder importiert werden können. Der Export dient dadurch nicht nur der Speicherung der Eingabedaten, sondern auch der Sicherung und Weitergabe bereits durchgeführter Simulationen.
...
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}X}
...
Die folgenden Listen fassen die zuvor hergeleiteten Anforderungen für die weitere Rückverfolgbarkeit im Systementwurf und in der Qualitätssicherung zusammen.
...
\begin{itemize}
    ...
    \item \textbf{FR-04:} Konfiguration verfahrensspezifischer Parameter (Zeitquantum, Tie-Break-Regel, Queue-Stufen und Basisquantum).
    ...
    \item \textbf{FR-11:} Speichern und Laden von Szenarien innerhalb der Anwendung.
    \item \textbf{FR-12:} Export eines Szenarios einschließlich der zugehörigen Simulationsläufe als JSON-Datei.
    \item \textbf{FR-13:} Import zuvor exportierter Szenarien einschließlich ihrer Simulationsläufe.
\end{itemize}
